...
Aside from CrashReport_Leopard.o and CrashReport_Snowleopard.o, which contain proprietary code for creating crash logs, CrashWrangler is licensed as sample code and source code is included. 
...
- automate.rb, which is a script which automates running fuzzing test cases with exc_handler.  Run it with the -h option for usage.

- A number of sample shell scripts in the sample_scripts directory, to be used with automate.rb to test specific applications.
...
- patch.rb and its helper FuzzPatch.rb, which are used to apply a binary patch to a file (useful when fuzzing very large files).
...
========= Build / install =========
make
[Optionally, make tests && ruby test_suite.rb.  Note the test suite is not supported on PPC processors or on Leopard].
cp exc_handler *.rb [the directory where you are running test cases]
Note: for convenience, pre-built exc_handler_leopard and exc_handler_snowleopard binaries are in the binaries subdirectory.  To use them, rename the one for your platform to exc_handler and use it as above.
...
What you want to look at is the is_exploitable field.
...
Generate a bunch of fuzzed movie files in a directory.  They should have the naming convention 00000000.mov, 00000001.mov, etc.

Assuming you have 10000 test cases in the directory named evaluation_cases
Run
./automate.rb 0 10000 sample_scripts/run_qt.sh .mov
...
If you are going to kill the program, you need to make sure that the file named cw.lck (or the file which was specified by the CW_LOCK_FILE environment variable) does not exist before you kill the program.
...
Otherwise return the signal number which caused the crash, described in signal(2) manpage.
...
CW_CURRENT_CASE: 
Some identifier for the case that's currently being run.
This is suitable for being used when the program is killed and relaunched for each case.
...
automate.rb includes support for binary diff files, which can be used instead of mutated files if the files would be prohibitively large.
The format is specified in FuzzPatch.rb.

To use this feature:
Your fuzzer should generate diff files in the format, with the name scheme like:
00000000.mov.diff, 00000001.mov.diff, etc

Then run
./automate.rb 0 100 ./run_qt.sh .mov.diff

automate.rb will apply the patch to the original file specified in the diff file, then create a temporary file at the output file specified in the diff.  (e.g. 00000000.mov).  It calls ./run_qt.mov 00000000.mov, and deletes 00000000.mov afterwards.   If the file caused a crash, later you can regenerate 00000000.mov by running ./patch.rb evaluation_cases/00000000.mov.diff