...
#if ( __MigTypeCheck || __NDR_convert__ )
#if __MIG_check__Reply__mach_exc_subsystem__
#if !defined(__MIG_check__Reply__transfer_ports_t__defined)
#define __MIG_check__Reply__transfer_ports_t__defined

mig_internal kern_return_t __MIG_check__Reply__transfer_ports_t(__Reply__transfer_ports_t *Out0P)
{

	typedef __Reply__transfer_ports_t __Reply;
...
#if	__MigTypeCheck
	if (Out0P->exception_port.type != MACH_MSG_PORT_DESCRIPTOR ||
	    Out0P->exception_port.disposition != 17) {
		return MIG_TYPE_ERROR;
	}
#endif	/* __MigTypeCheck */

#if	__MigTypeCheck
	if (Out0P->orig_bootstrap_port.type != MACH_MSG_PORT_DESCRIPTOR ||
	    Out0P->orig_bootstrap_port.disposition != 17) {
		return MIG_TYPE_ERROR;
	}
#endif	/* __MigTypeCheck */
...
#endif /* !defined(__MIG_check__Reply__transfer_ports_t__defined) */
#endif /* __MIG_check__Reply__mach_exc_subsystem__ */
#endif /* ( __MigTypeCheck || __NDR_convert__ ) */


/* Routine transfer_ports */
mig_external kern_return_t transfer_ports
(
	mach_port_t server,
	mach_port_t *exception_port,
...
#pragma pack()
...
#ifdef	__MIG_check__Reply__transfer_ports_t__defined
	kern_return_t check_result;
#endif	/* __MIG_check__Reply__transfer_ports_t__defined */

	__DeclareSendRpc(2408, "transfer_ports")
	...
	__BeforeSendRpc(2408, "transfer_ports")
	msg_result = mach_msg(&InP->Head, MACH_SEND_MSG|MACH_RCV_MSG|MACH_MSG_OPTION_NONE, (mach_msg_size_t)sizeof(Request), (mach_msg_size_t)sizeof(Reply), InP->Head.msgh_reply_port, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
	__AfterSendRpc(2408, "transfer_ports")
	if (msg_result != MACH_MSG_SUCCESS) {
		__MachMsgErrorWithoutTimeout(msg_result);
		{ return msg_result; }
	}
...
#if	defined(__MIG_check__Reply__transfer_ports_t__defined)
	check_result = __MIG_check__Reply__transfer_ports_t((__Reply__transfer_ports_t *)Out0P);
	if (check_result != MACH_MSG_SUCCESS)
		{ return check_result; }
#endif	/* defined(__MIG_check__Reply__transfer_ports_t__defined) */

	*exception_port = Out0P->exception_port.name;
	*orig_bootstrap_port = Out0P->orig_bootstrap_port.name;
	return KERN_SUCCESS;
}
