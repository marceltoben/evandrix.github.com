...
/*  socklen_t fromlen;
  ...
  if ((p = thc_pcap_init_promisc(interface, "ip6 and udp and (dst port 53 or dst port 5355)")) == NULL) {
...
/*
  ...
  if ((t = thc_bind_udp_port(5355)) < 0) {
    fprintf(stderr, "Error: could not bind to 5355/udp\n");
    exit(-1);
  } else {
