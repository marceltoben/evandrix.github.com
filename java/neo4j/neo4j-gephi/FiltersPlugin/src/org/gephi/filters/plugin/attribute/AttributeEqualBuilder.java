...
public class AttributeEqualBuilder implements CategoryBuilder {
    ...
    public static class EqualNumberFilter implements Filter {
        ...
        public boolean evaluate(Graph graph, Node node) {
            ...
            if (val != null) {
                values.add(val);
                //will work? if we compare Integer.equals(Long) we get false!!!
                //maybe val should be cast to Number and compare their primitive values?
                //((Number) val).doubleValue() == match.doubleValue();
