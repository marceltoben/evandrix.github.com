...
//        addNodeToDelegateStorageEngine(node);

        return node;
    }

    private void addNodeToDelegateStorageEngine(Node node) {
        AttributeTable nodeTable =
                Lookup.getDefault().lookup(AttributeController.class).getModel().getNodeTable();

        Collection<AttributeValueDelegateProvider<?>> providers = nodeTable.getProviders();

        switch (providers.size()) {
            case 0:
                break;//no providers means no addings to delegate storage engine
            case 1:
                GraphItemDelegateFactoryProvider<?> graphItemDelegateFactoryProvider =
                    nodeTable.getProviders().iterator().next().graphItemDelegateFactoryProvider();
//                graphItemDelegateFactoryProvider.
//TODO finish

            default:
                throw new IllegalStateException("Multiple delegate providers not supported yet.");
        }
    }
